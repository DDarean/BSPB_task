...
class Task3solver:
    ...
    def __init__(self, month: int, min_payment: int, cb_size: int):
        ...
        self.clients_table: pd.DataFrame
        self.cashback_table: pd.DataFrame
    ...
    def check_for_cashback(self) -> None:
        ...
        for i in range(1, len(cb_table)):
            if (
                (cb_table.loc[i, "month"] >= self.month)
                and (cb_table.loc[i, "previous"])
                and not (cb_table.loc[i - 1, "cashback"])
            ):
