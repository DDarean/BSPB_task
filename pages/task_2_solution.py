...
st.write(
    ...
    "1. после подготовки датасета - начальное обучение и подбор "
    "гиперпараметров при помощи grid search и кросс-валидации  \n"
    "2. после этого проиходит отбор признаков и повторное обучение "
    "модели только на отобранных признаках  \n"
    "3. Для предсказаний модель обучается на всех доступных данных "
    "с параметрами, подобранными на предыдущем шаге  \n"
...
path_to_submit = st.text_input(
    label="Path to data file for predictions", value="data/task2_submit.csv"
)
...
if path_to_train:
    ...
    if st.button(label="RUN"):
        ...
        st.write(
            f"Metrics on top features and best parameters  \n"
            f"f1: {f1_t:.4f}, "
            f"precision: {precision_t:.4f}, "
            f"recall: {recall_t:.4f}"
        )
        with st.spinner("Making predictions"):
            preds = solver.retrain_and_predict(path_to_train, path_to_submit)
